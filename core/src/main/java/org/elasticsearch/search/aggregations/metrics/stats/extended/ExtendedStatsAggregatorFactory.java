...
public class ExtendedStatsAggregatorFactory extends ValuesSourceAggregatorFactory<ValuesSource.Numeric, ExtendedStatsAggregatorFactory> {
    ...
    protected Aggregator createUnmapped(Aggregator parent) throws IOException {
        return new ExtendedStatsAggregator(name, null, config.format(), context, parent, sigma);
    }
    ...
    protected Aggregator doCreateInternal(ValuesSource.Numeric valuesSource, Aggregator parent, boolean collectsFromSingleBucket)
            throws IOException {
        return new ExtendedStatsAggregator(name, valuesSource, config.format(), context, parent, sigma);
    }
}
